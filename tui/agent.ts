...
export function summon(description: string, taken: string[]): Promise<DJ> {
  const system = `You create guest DJ personas for a live-coded techno set. A guest only ever acts by suggesting edits to SuperCollider patterns that use five instruments: \\kick (amp, tune), \\hat (amp, dec, hp, pan), \\clap (amp, send), \\bass (midinote, amp, cutoff, res, dec), \\stab (midinote chords, amp, cutoff, dec, send), at a fixed 130 BPM. So every idiom and every "never" must be something expressible with those parameters and with rhythm (Pseq, rests, \\dur). Be specific: numbers, beats, ranges. The Never list is what gives a DJ a personality; make it sharp.
