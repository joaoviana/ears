...
export const AMBIENT_ARSENAL: AmbientSeed[] = [
  { id: "fingertips", label: "close fingertips", slot: "d6", why: "Slowed fingertips land close, woody and tactile without becoming a drum loop.", expect: { metric: "low", dir: "down" },
    code: '~d.(\\d6, \\instrument, \\texture, \\buf, ~t.(\\fingertips), \\delta, Pseq([0.75, 0.5, 2.5, 1.25, 4, 0.5, 6], inf), \\dur, 4, \\len, Pwhite(3.2, 5.4), \\att, 0.012, \\rel, 0.65, \\start, Pwhite(0, 0.18), \\rate, Pwrand([0.52, 0.63, 0.74], [0.25, 0.5, 0.25], inf), \\hp, 110, \\lp, 6200, \\sub, Pwhite(0.12, 0.26), \\subfreq, Pwhite(64, 90), \\amp, Pwhite(0.1, 0.15), \\pan, Pbrown(-0.55, 0.55, 0.12), \\send, 0.48)' },
  { id: "marbles", label: "slow marbles", slot: "d4", why: "Pitch-darkened marbles roll across wood, then leave ten seconds of space.", expect: { metric: "sub", dir: "up" },
    code: '~d.(\\d4, \\instrument, \\texture, \\buf, ~t.(\\marbles), \\delta, Pseq([1.5, 0.6, 2.8, 1, 6, 0.75, 9], inf), \\dur, 5, \\len, Pwhite(4.2, 6), \\att, 0.018, \\rel, 0.9, \\start, Pwhite(0, 0.18), \\rate, Pwrand([0.38, 0.48, 0.62], [0.2, 0.55, 0.25], inf), \\hp, 90, \\lp, 5200, \\sub, Pwhite(0.14, 0.28), \\subfreq, Pwhite(56, 80), \\amp, Pwhite(0.1, 0.14), \\pan, Pbrown(-0.65, 0.65, 0.16), \\send, 0.56)' },
  ...
  { id: "brush-current", label: "forest brush current", slot: "d4", why: "A slowed forest brush becomes an uneven wooden current with breathing stereo depth.", expect: { metric: "low", dir: "down" },
  ...
  { id: "rain-prism", label: "rhythmic rain prism", slot: "d2", why: "The current voice breaks into bright, irregular rain-sized openings.", expect: { metric: "high", dir: "up" },
    ...
    code: '~d.(\\d2, \\instrument, \\cloud, \\buf, ~g.(\\rain), \\delta, Pexprand(15, 27, inf), \\dur, 20, \\rate, Pwrand([0.49, 0.74, 1], [0.18, 0.52, 0.3], inf), \\pos, Pbrown(0.02, 0.9, 0.06), \\wander, 0.2, \\grain, Pwhite(0.08, 0.28), \\density, Pwhite(30, 55), \\jitter, 0.16, \\att, 4, \\sus, 13, \\rel, 7, \\hp, 380, \\lp, 9200, \\spread, 1, \\shimmer, 0.1, \\amp, 0.15, \\send, 0.86)' },
  { id: "wave-memory", label: "wave breathing cycle", slot: "d1", why: "The current voice breathes in a five-part low-pass tide.", expect: { metric: "air", dir: "down" },
  ...
  { id: "paper-bloom", label: "paper bloom", slot: "d5", why: "Warm paper fibres fold close to the listener, then leave a generous gap.", expect: { metric: "air", dir: "up" },
    code: '~d.(\\d5, \\instrument, \\texture, \\buf, ~t.(\\paper), \\delta, Pseq([1.25, 0.6, 4.5, 2, 7.5, 0.8, 11], inf), \\dur, 5, \\len, Pwhite(3.2, 5.8), \\att, 0.025, \\rel, Pwhite(0.7, 1.2), \\start, Pbrown(0.03, 0.86, 0.06), \\rate, Pwrand([0.46, 0.58, 0.71], [0.2, 0.58, 0.22], inf), \\hp, 80, \\lp, Pseg([2200, 6500, 3000], 56, repeats: inf), \\amp, Pwhite(0.12, 0.18), \\pan, Pbrown(-0.68, 0.68, 0.15), \\send, 0.46)' },
  { id: "droplets", label: "submerged droplets", slot: "d6", why: "Close water taps carry a soft sub pulse and grainy skin.", expect: { metric: "air", dir: "down" },
  ...
  { id: "twig-cycle", label: "twig cycle", slot: "d6", why: "Dry branch taps trace a five-part cycle with one deliberate hole.", expect: { metric: "sub", dir: "down" },
    code: '~d.(\\d6, \\instrument, \\twig, \\delta, Pseq([1.25, 0.75, 3.5, 2, 5.5], inf), \\dur, 0.3, \\midinote, Pseq([62, 69, \\r, 66, 74], inf), \\dec, Pwhite(0.12, 0.34), \\tone, Pwhite(0.1, 0.4), \\hollow, Pwhite(0.35, 0.78), \\amp, Pwhite(0.04, 0.07), \\pan, Pbrown(-0.65, 0.65, 0.18), \\send, 0.7)' },
  { id: "pebble-pairs", label: "pebble pairs", slot: "d6", why: "Two close pebbles answer, followed by an unpredictable stretch of water.", expect: { metric: "air", dir: "down" },
  ...
  { id: "leaf-shuffle", label: "leaf shuffle", slot: "d4", why: "Close leaves make an uneven soft shuffle under the field recordings.", expect: { metric: "sub", dir: "down" },
    code: '~d.(\\d4, \\instrument, \\rustle, \\delta, Pseq([0.75, 1.5, 0.5, 2.25, 1, 3.5, 0.5, 5], inf), \\dur, 1.5, \\freq, Pbrown(1100, 4200, 320), \\dec, Pwhite(0.45, 1.4), \\grain, Pwhite(8, 38), \\bw, Pwhite(0.25, 0.75), \\amp, Pwhite(0.1, 0.16), \\pan, Pbrown(-0.68, 0.68, 0.14), \\send, 0.74)' },
  { id: "reed-answer", label: "hollow reed answers", slot: "d5", why: "Hollow reeds answer the water in a spacious, asymmetric phrase.", expect: { metric: "loudness", dir: "down" },
    code: '~d.(\\d5, \\instrument, \\twig, \\delta, Pseq([2, 1, 4.5, 3, 6.5], inf), \\dur, 0.4, \\midinote, Pseq([57, \\r, 64, 61, 68], inf), \\dec, Pwhite(0.22, 0.48), \\tone, Pwhite(0.08, 0.32), \\hollow, Pwhite(0.68, 0.95), \\amp, Pwhite(0.04, 0.065), \\pan, Pseq([-0.45, 0.2, 0.55, -0.1, 0.35], inf), \\send, 0.82)' },
  // The harmonic gestures use \glow, the warm voice: a sine, a soft second harmonic and low-passed saws with slow
  // per-voice drift. The old \pad here was four saws through a resonant filter at 0.032, which was both buzzy and
  // inaudible, and the old clearing was Gendy sine tones alternating D with G# (a bare tritone, five seconds long):
  // exactly the "sparse high notes with long tails" the brief itself calls ominous.
  { id: "halo", label: "warm glow", slot: "d5", why: "Slow D-lydian warmth rises under the field, holds, and goes out again like light through cloud.", expect: { metric: "sub", dir: "down" },
    transform: [{ key: "delta", value: "Pseq([3, 1, 5, 2, 8], inf)" }, { key: "fxhp", value: "110" }, { key: "fxlp", value: "4400" }, { key: "fxdrive", value: "0.08" }, { key: "fxspace", value: "0.5" }, { key: "fxmotion", value: "0.62" }, { key: "fxrate", value: "0.075" }],
    code: '~d.(\\d5, \\instrument, \\glow, \\delta, Pexprand(12, 24, inf), \\midinote, Pseq([[50, 57, 64, 69], [50, 59, 64, 68], [50, 57, 61, 66], [50, 56, 59, 64], [50, 57, 62, 69], [45, 52, 61, 64]], inf), \\att, Pwhite(6, 10), \\sus, Pwhite(12, 20), \\rel, Pwhite(10, 16), \\warm, Pwhite(2.4, 4), \\breath, Pwhite(0.05, 0.2), \\shine, Pwhite(0.2, 0.5), \\swell, Pwhite(0.25, 0.5), \\saw, Pwhite(0.3, 0.6), \\amp, Pwrand([Pwhite(0.028, 0.042, 1), Rest(0)], [0.9, 0.1], inf), \\pan, Pwhite(-0.25, 0.25), \\send, 0.6)' },
  // Four more of the voice the performer asked for by name, in four registers, so the rotation reaches one often.
  { id: "organ-tide", label: "space organ tide", slot: "d5", why: "The glow becomes a wide saw organ: the chord cycle swells, opens, and breathes over the field.", expect: { metric: "air", dir: "up" },
    code: '~d.(\\d5, \\instrument, \\glow, \\delta, Pexprand(10, 18, inf), \\midinote, Pseq([[50, 57, 64, 69], [50, 59, 64, 68], [50, 57, 61, 66], [50, 56, 59, 64], [50, 57, 62, 69], [45, 52, 61, 64]], inf), \\att, Pwhite(7, 12), \\sus, Pwhite(14, 24), \\rel, Pwhite(10, 16), \\warm, Pseg([2, 5.5, 2.4], 60, repeats: inf), \\breath, 0.08, \\shine, Pwhite(0.3, 0.6), \\swell, Pwhite(0.35, 0.6), \\saw, Pwhite(0.65, 0.9), \\amp, Pwhite(0.024, 0.036), \\pan, Pwhite(-0.2, 0.2), \\send, 0.8)' },
  { id: "star-field", label: "star field", slot: "d3", why: "High, quiet glow chords glimmer far above the canopy, each one taking ten seconds to arrive.", expect: { metric: "mid", dir: "up" },
    code: '~d.(\\d3, \\instrument, \\glow, \\delta, Pexprand(14, 30, inf), \\midinote, Pwrand([[69, 76, 81], [69, 74, 80], [71, 76, 83], [66, 73, 78, 81], \\r], [0.26, 0.22, 0.2, 0.18, 0.14], inf), \\att, Pwhite(8, 13), \\sus, Pwhite(8, 16), \\rel, Pwhite(10, 18), \\warm, Pwhite(1.6, 2.4), \\breath, Pwhite(0.2, 0.4), \\shine, Pwhite(0.5, 0.8), \\swell, 0.5, \\saw, Pwhite(0.15, 0.35), \\amp, Pwhite(0.012, 0.02), \\pan, Pwhite(-0.7, 0.7), \\send, 0.9)' },
  { id: "deep-orbit", label: "deep orbit", slot: "d4", why: "A low saw pedal on D2 and A2 arrives from underneath, holds twenty seconds, and leaves.", expect: { metric: "sub", dir: "up" },
    code: '~d.(\\d4, \\instrument, \\glow, \\delta, Pexprand(24, 44, inf), \\midinote, Pwrand([[38, 45], [38, 50], [45, 52], [33, 45]], [0.35, 0.25, 0.25, 0.15], inf), \\att, Pwhite(6, 9), \\sus, Pwhite(10, 16), \\rel, Pwhite(8, 12), \\warm, Pwhite(2, 3), \\breath, 0.04, \\shine, 0.1, \\swell, 0.3, \\saw, Pwhite(0.4, 0.6), \\amp, Pwhite(0.03, 0.045), \\pan, 0, \\send, 0.6)' },
  { id: "saw-dawn", label: "saw dawn", slot: "d6", why: "Bright saw chords open from a closed filter to full light across a minute, then close again.", expect: { metric: "sub", dir: "down" },
    code: '~d.(\\d6, \\instrument, \\glow, \\delta, Pexprand(8, 16, inf), \\midinote, Pseq([[57, 64, 69, 73], [57, 62, 69, 76], [59, 64, 71, 76], [57, 61, 66, 73]], inf), \\att, Pwhite(5, 9), \\sus, Pwhite(10, 18), \\rel, Pwhite(8, 14), \\warm, Pseg([1.5, 7, 2], 64, repeats: inf), \\breath, 0.1, \\shine, Pwhite(0.3, 0.5), \\swell, 0.4, \\saw, Pwhite(0.75, 1), \\amp, Pwhite(0.016, 0.026), \\pan, Pwhite(-0.5, 0.5), \\send, 0.85)' },
  { id: "clearing", label: "rhythmic clearing", slot: "d4", why: "The current floor opens into irregular filtered windows and real silence.", expect: { metric: "air", dir: "down" },
    ...
    code: '~d.(\\d4, \\instrument, \\glow, \\delta, Pexprand(18, 40, inf), \\midinote, Pwrand([[45, 52], [50, 57], [45, 54], \\r, \\r], [0.2, 0.18, 0.14, 0.24, 0.24], inf), \\att, Pwhite(4, 7), \\sus, Pwhite(3, 6), \\rel, Pwhite(6, 10), \\warm, Pwhite(2, 3), \\breath, 0.04, \\amp, Pwhite(0.035, 0.055), \\pan, Pwhite(-0.4, 0.4), \\send, 0.7)' },
  { id: "canopy", label: "canopy shutters", slot: "d3", why: "The current voice flickers through distant, slowly moving canopy bands.", expect: { metric: "loudness", dir: "down" },
    ...
    code: '~d.(\\d3, \\instrument, \\nature, \\buf, ~n.(\\birds), \\dur, 48, \\len, 40, \\att, 12, \\rel, 14, \\start, Pwhite(0.08, 0.78), \\rate, Pwhite(0.72, 0.96), \\hp, 1200, \\lp, 8500, \\amp, 0.095, \\pan, Pbrown(-0.7, 0.7, 0.16), \\send, 0.88)' },
  { id: "mist", label: "tactile mist pulse", slot: "d6", why: "The current detail smears into soft pulses, then snaps back into focus.", expect: { metric: "loudness", dir: "down" },
  ...
  { id: "air-flutter", label: "air flutter", slot: "d3", why: "Unpitched leaf air flickers high, bunches briefly, then vanishes.", expect: { metric: "air", dir: "up" },
    code: '~d.(\\d3, \\instrument, \\rustle, \\delta, Pseq([0.22, 0.35, 0.18, Pexprand(5, 12, 1)], inf), \\dur, 1.2, \\freq, Pbrown(3800, 7200, 540), \\dec, Pwhite(0.25, 0.75), \\grain, Pwhite(32, 68), \\bw, Pwhite(0.32, 0.7), \\amp, Pwhite(0.085, 0.14), \\pan, Pbrown(-0.85, 0.85, 0.24), \\send, 0.68)' },
  ...
  { id: "brush-burst", label: "brush burst", slot: "d4", why: "Close brush fibres make short asymmetric flurries, then leave the field bare.", expect: { metric: "loudness", dir: "down" },
    code: '~d.(\\d4, \\instrument, \\texture, \\buf, ~t.(\\brush), \\delta, Pseq([0.2, 0.28, 0.46, Pexprand(4.5, 10, 1), 0.3, Pexprand(7, 14, 1)], inf), \\dur, 2.4, \\len, Pwhite(1.8, 3.5), \\att, 0.01, \\rel, Pwhite(0.35, 0.8), \\start, Pbrown(0.04, 0.88, 0.09), \\rate, Pwrand([0.5, 0.68, 0.9], [0.25, 0.5, 0.25], inf), \\hp, 110, \\lp, 7600, \\amp, Pwhite(0.1, 0.15), \\pan, Pbrown(-0.82, 0.82, 0.2), \\send, 0.5)' },
  { id: "bird-grain", label: "bird grain eddies", slot: "d3", why: "Bird fragments gather into granular eddies, rise, and scatter back into canopy.", expect: { metric: "loudness", dir: "up" },
  ...
  { id: "paper-fall", label: "paper fall", slot: "d6", why: "Paper grains tumble in changing clusters across the stereo field, never repeating.", expect: { metric: "brightness", dir: "up" },
    code: '~d.(\\d6, \\instrument, \\texture, \\buf, ~t.(\\paper), \\delta, Pseq([0.32, 0.7, 0.24, 1.6, Pexprand(4, 10, 1)], inf), \\dur, 2.8, \\len, Pwhite(1.8, 4.2), \\att, 0.012, \\rel, Pwhite(0.4, 0.9), \\start, Pwhite(0.02, 0.9), \\rate, Pwhite(0.42, 0.92), \\hp, Pwhite(140, 480), \\lp, Pwhite(4800, 9800), \\amp, Pwhite(0.12, 0.18), \\pan, Pwhite(-0.9, 0.9), \\send, 0.58)' },
  { id: "wave-shutters", label: "wave shutters", slot: "d1", why: "The waves alternate between deep body and bright spray in irregular openings.", expect: { metric: "loudness", dir: "up" },
  ...
  { id: "rain-polyrhythm", label: "rain cross-cycle", slot: "d2", why: "Rain answers itself in short threes against a slower five-gap current.", expect: { metric: "loudness", dir: "up" },
  ...
  { id: "open-horizon", label: "open horizon", slot: "d5", why: "The middle falls away, then returns as a wide moving band.", expect: { metric: "low", dir: "down" },
  ...
  // The extremes stay extreme in TIME (rare, sudden, absent) but no longer in menace. Measured, the old lows were
  // 37-55 Hz sines with a pitch drop, reading sub -19 dBFS against body -35 ("muddy: sub over body, no air"), which
  // is the horror-cue sound. A low that arrives and leaves is kept, in the D2/A2 register where it is warm. The twig
  // voices are impulse-excited and tanh-limited, so a twig at amp 0.09 peaks near -20 dBFS in the 2-8k band: over the
  // bed by 6 dB and reading as clatter. Twig amps and tone sit lower now; the burst-then-hole shape is unchanged.
  { id: "tide-floor", label: "deep tide floor", slot: "d4", why: "A real low arrives from nowhere, holds the room for six seconds, and leaves it empty again.", expect: { metric: "sub", dir: "up" },
    code: '~d.(\\d4, \\instrument, \\sub, \\delta, Pseq([Pexprand(13, 24, 1), 2.5, Pexprand(21, 38, 1)], inf), \\dur, 8, \\midinote, Pwrand([38, 45, 33], [0.46, 0.3, 0.24], inf), \\dec, Pwhite(4, 7), \\drop, Pwhite(0.03, 0.14), \\duck, 0, \\amp, Pwhite(0.04, 0.065))' },
  { id: "spray", label: "bright spray", slot: "d2", why: "The waves are played back above their own pitch until only spray is left: the top of the room, briefly.", expect: { metric: "air", dir: "up" },
    code: '~d.(\\d2, \\instrument, \\cloud, \\buf, ~g.(\\waves), \\delta, Pseq([2, 0.75, 5, Pexprand(9, 19, 1)], inf), \\dur, 7, \\rate, Pwrand([1.2, 1.5, 1.8], [0.3, 0.45, 0.25], inf), \\pos, Pwhite(0.05, 0.92), \\wander, 0.24, \\grain, Pwhite(0.05, 0.18), \\density, Pwhite(26, 54), \\jitter, 0.16, \\att, 1.6, \\sus, 3.5, \\rel, 3, \\hp, 2600, \\lp, 10500, \\spread, 1, \\shimmer, 0.1, \\amp, 0.12, \\send, 0.6)' },
  { id: "swarm", label: "twig swarm", slot: "d6", why: "Five or six dry impacts in under a beat, then twenty seconds of nothing. The gap is the gesture.", expect: { metric: "sub", dir: "down" },
    code: '~d.(\\d6, \\instrument, \\twig, \\delta, Pwrand([0.125, 0.1875, 0.25, Pexprand(16, 34, 1)], [0.3, 0.26, 0.26, 0.18], inf), \\dur, 0.25, \\midinote, Pwhite(50, 70), \\dec, Pexprand(0.08, 0.26, inf), \\tone, Pwhite(0.08, 0.3), \\hollow, Pwhite(0.45, 0.9), \\amp, Pwhite(0.025, 0.05), \\pan, Pwhite(-0.9, 0.9), \\send, 0.62)' },
  { id: "thunder-stone", label: "low stone", slot: "d5", why: "A soft low body rolls in twice a minute, touches the floor, then disappears.", expect: { metric: "sub", dir: "up" },
    code: '~d.(\\d5, \\instrument, \\stone, \\delta, Pseq([Pexprand(18, 34, 1), 3.5], inf), \\dur, 7, \\tune, Pwrand([55, 73, 92], [0.42, 0.34, 0.24], inf), \\dec, Pwhite(2, 4), \\bend, Pwhite(0.08, 0.3), \\skin, Pwhite(0.18, 0.46), \\tone, Pwhite(0.12, 0.38), \\amp, Pwhite(0.04, 0.07), \\pan, Pwhite(-0.4, 0.4), \\send, 0.5)' },
  { id: "falling-air", label: "falling air", slot: "d3", why: "A band of air opens high and slides downward as it fades, like weather passing over the field.", expect: { metric: "loudness", dir: "up" },
    code: '~d.(\\d3, \\instrument, \\noise, \\delta, Pseq([Pexprand(11, 22, 1), 4.5, Pexprand(16, 30, 1)], inf), \\dur, 7, \\freq, Pwhite(3200, 6000), \\att, Pwhite(2, 4), \\dec, Pwhite(4, 8), \\bw, Pwhite(0.3, 0.7), \\sweep, Pwhite(0.3, 0.55), \\amp, Pwhite(0.04, 0.07), \\pan, Pbrown(-0.8, 0.8), \\send, 0.8)' },
  { id: "glint", label: "rare glint", slot: "d3", why: "One unpitched leaf-air event appears every ten to forty-five seconds.", expect: { metric: "density", dir: "up" },
    code: '~d.(\\d3, \\instrument, \\rustle, \\delta, Pexprand(14, 46, inf), \\dur, 2.5, \\freq, Pwhite(3600, 6800), \\dec, Pwhite(0.8, 2), \\grain, Pwhite(38, 76), \\bw, Pwhite(0.35, 0.8), \\sweep, Pwhite(0.7, 1.3), \\air, Pwhite(0.5, 0.85), \\amp, Pwhite(0.08, 0.13), \\pan, Pwhite(-0.95, 0.95), \\send, 0.72)' },
  { id: "hollow-out", label: "hollow out", slot: "d2", why: "The chosen voice stops being a layer and becomes an event every thirty seconds, behind a soft filter.", expect: { metric: "loudness", dir: "down" },
    transform: [{ key: "delta", value: "Pexprand(24, 48, inf)" }, { key: "fxhp", value: "60" }, { key: "fxlp", value: "Pseg([1100, 2600, 900], 40, repeats: inf)" }, { key: "fxmotion", value: "0.5" }, { key: "fxrate", value: "0.03" }, { key: "fxspace", value: "0.5" }],
    code: '~d.(\\d2, \\instrument, \\cloud, \\buf, ~g.(\\rain), \\delta, Pexprand(24, 48, inf), \\dur, 16, \\rate, Pwhite(0.45, 0.7), \\pos, Pbrown(0.05, 0.9, 0.04), \\wander, 0.08, \\grain, Pwhite(0.4, 1.2), \\density, Pwhite(4, 9), \\jitter, 0.05, \\att, 5, \\sus, 7, \\rel, 6, \\hp, 60, \\lp, 2600, \\spread, 0.9, \\shimmer, 0.03, \\amp, 0.09, \\send, 0.8)' },
];

// Measured over the base with a muted engine (dev/measure-expect): the metric each gesture moves most, and how
...
export const MEASURED: Record<string, { metric: string; dir: string; strength: number }> = {
  "air-flutter": { metric: "air", dir: "up", strength: 30.12 },
  "bird-grain": { metric: "loudness", dir: "up", strength: 104.44 },
  "brush-burst": { metric: "loudness", dir: "down", strength: 26.75 },
  "brush-current": { metric: "low", dir: "down", strength: 12.51 },
  "canopy": { metric: "loudness", dir: "down", strength: 6.61 },
  "clearing": { metric: "air", dir: "down", strength: 12.08 },
  "deep-orbit": { metric: "sub", dir: "up", strength: 9.95 },
  "droplets": { metric: "air", dir: "down", strength: 23.83 },
  "falling-air": { metric: "loudness", dir: "up", strength: 90.15 },
  "fingertips": { metric: "low", dir: "down", strength: 8.03 },
  "glint": { metric: "density", dir: "up", strength: 2.08 },
  "halo": { metric: "sub", dir: "down", strength: 1.23 },
  "hollow-out": { metric: "loudness", dir: "down", strength: 72.9 },
  "leaf-shuffle": { metric: "sub", dir: "down", strength: 29.11 },
  "marbles": { metric: "sub", dir: "up", strength: 17.79 },
  "mist": { metric: "loudness", dir: "down", strength: 21.29 },
  "open-horizon": { metric: "low", dir: "down", strength: 2.55 },
  "organ-tide": { metric: "air", dir: "up", strength: 2.65 },
  "paper-aurora": { metric: "brightness", dir: "up", strength: 7.69 },
  "paper-bloom": { metric: "air", dir: "up", strength: 10.53 },
  "paper-fall": { metric: "brightness", dir: "up", strength: 14.88 },
  "pebble-pairs": { metric: "air", dir: "down", strength: 21.25 },
  "rain-polyrhythm": { metric: "loudness", dir: "up", strength: 13.2 },
  "rain-prism": { metric: "high", dir: "up", strength: 5.47 },
  "reed-answer": { metric: "loudness", dir: "down", strength: 7.94 },
  "saw-dawn": { metric: "sub", dir: "down", strength: 21.6 },
  "spray": { metric: "air", dir: "up", strength: 16.63 },
  "star-field": { metric: "mid", dir: "up", strength: 23.69 },
  "swarm": { metric: "sub", dir: "down", strength: 17.59 },
  "thunder-stone": { metric: "sub", dir: "up", strength: 11.71 },
  "tide-floor": { metric: "sub", dir: "up", strength: 8.64 },
  "twig-cycle": { metric: "sub", dir: "down", strength: 27.44 },
  "understone": { metric: "sub", dir: "up", strength: 1.46 },
  "wave-memory": { metric: "air", dir: "down", strength: 9.28 },
  "wave-shutters": { metric: "loudness", dir: "up", strength: 9.18 },
};
...
const ROTATION = ["fingertips", "marbles", "air-flutter", "tide-floor", "understone", "paper-aurora", "swarm",
  ...
  "thunder-stone", "organ-tide", "rain-polyrhythm", "leaf-shuffle", "canopy", "star-field", "glint", "droplets", "open-horizon", "clearing",
  "wave-memory", "deep-orbit", "falling-air", "paper-fall", "mist", "saw-dawn", "pebble-pairs", "brush-current", "reed-answer", "halo"];
...
const CONTOUR: Record<string, Contour> = {
  ...
  "organ-tide": { energy: 1, band: "mid" }, "star-field": { energy: 0.5, band: "high" },
  "deep-orbit": { energy: 1.5, band: "low" }, "saw-dawn": { energy: 1, band: "high" },
};
export const contour = (id: string): Contour => CONTOUR[id] ?? { energy: 0.5, band: "mid" };
...
const MATERIAL_ADDITIONS = ["organ-tide", "star-field", "air-flutter", "understone", "paper-fall", "brush-burst", "fingertips", "swarm", "paper-bloom", "leaf-shuffle", "falling-air", "brush-current", "twig-cycle", "tide-floor", "marbles", "glint", "paper-aurora", "spray", "canopy", "thunder-stone"];
...
const FILTER_MOVES = [
  ...
  // one takes a voice down behind a soft door, the other opens the whole spectrum at once. The door used to be a
  // 400 Hz low-pass in a big wash: a muffled rumble from the next room, the one filter move that reliably reads as dread.
  { phrase: "settles behind a soft door", set: [{ key: "fxhp", value: "60" }, { key: "fxlp", value: "Pseg([900, 1900, 800], 44, repeats: inf)" }, { key: "fxmotion", value: "0.5" }, { key: "fxrate", value: "0.03" }, { key: "fxspace", value: "0.5" }] },
  { phrase: "throws the whole spectrum open at once", set: [{ key: "fxhp", value: "Pseg([28, 45, 30], 36, repeats: inf)" }, { key: "fxlp", value: "Pseg([14000, 3200, 15000], 30, repeats: inf)" }, { key: "fxmotion", value: "0.3" }, { key: "fxrate", value: "0.26" }, { key: "fxdrive", value: "0.1" }, { key: "fxspace", value: "0.14" }] },
];

type MovePart = Suggestion["parts"][number];

// The glow gestures stack: taking one of them brings a second glow voice in another register with it, so a chord
// is layered across the room (a low pedal under an organ, stars over a dawn) the way the performer asked for.
const GLOW_STACK = ["organ-tide", "star-field", "deep-orbit", "saw-dawn", "halo"];
...
function companion(seed: AmbientSeed, input: AskInput, transformed: boolean, excluded: Set<string>, blocked = new Set<string>()): { part: MovePart; phrase: string; evidence: string } | null {
  if (GLOW_STACK.includes(seed.id) && !transformed) {
    const candidates = GLOW_STACK.filter((id) => id !== seed.id).map((id) => AMBIENT_ARSENAL.find((item) => item.id === id)!)
    ...
    for (let offset = 0; offset < candidates.length; offset++) {
      const layer = candidates[(start + offset) % candidates.length], before = input.slots[layer.slot] || "";
      if (before.trim() === layer.code.trim() || validate({ slot: layer.slot, code: layer.code })) continue;
      return { part: { slot: layer.slot, code: layer.code, diff: `stack · ${layer.label}` },
        phrase: `${layer.label} stacks with it in ${layer.slot}`, evidence: `${seed.slot} and ${layer.slot} are one chord across two registers` };
    }
  }
  if (transformed) {
...
export function ambientArsenal(input: AskInput, count = 2): Suggestion[] {
  ...
  const family = (id: string) => Math.abs(contour(id).energy) >= 1.5 ? "contrast"
    ...
    : /rain|wave|canopy|mist|bird/.test(id) ? "weather" : /paper|halo|air|organ|star|orbit|dawn/.test(id) ? "light" : "space";
  ...
  const harmonicNote = /harmon|chord|glow|organ|saw|synth|interstellar|pad|warm|stack|layer/.test(note);
  // The library is a lookup, not a listener. A note it cannot place in any family gets no instant answer at all:
  // the model seat answers the words, and the pane says so, instead of a generic move dressed up as a reply.
  const families: [string, boolean][] = [["rhythm", rhythmic], ["filter", filtering], ["texture", tactile], ["random", stochastic], ["high", wantsHigh], ["low", wantsLow], ["synth", harmonicNote]];
  const matched = families.filter(([, hit]) => hit).map(([name]) => name);
  if (note.trim() && !matched.length) return [];
  const pool = Array.from({ length: ordered.length }, (_, offset) => {
    ...
    const isHigh = /air|bird|paper|rain|star|dawn/.test(seed.id), isLow = /stone|marble|droplet|wave|orbit/.test(seed.id);
    const harmonic = harmonicNote && /halo|organ|star|orbit|dawn/.test(seed.id);
    // a listener's own repertoire outranks the shared rotation; its calling card outranks everything on arrival
    // the repertoire cycles: this round's own gesture leads, so a listener works through its set rather than repeating its best one
    const sig = input.dj.signature ?? [], own = sig.includes(seed.id) ? 2.2 + (sig[(input.round ?? 0) % Math.max(1, sig.length)] === seed.id ? 1.2 : 0) : 0, card = input.entrance === seed.id ? 20 : 0;
    // a gesture whose measured effect clears the meter's floor makes a round the room can see checked
    const checkable = (MEASURED[seed.id]?.strength ?? 0) >= 1.5 ? 0.15 : 0;
    // a slot whose last change is still waiting for its check is left alone, so the check can finish
    const pending = input.pendingSlots?.includes(seed.slot) ? -2.5 : 0;
    return { seed, score: learned + own + card + checkable + pending - offset * 0.035 + (family(seed.id) === discipline ? (discipline === "contrast" ? 0.7 : 0.13) : 0)
      ...
      + (wantsHigh && isHigh ? 1.1 : 0) + (wantsLow && isLow ? 1.1 : 0) + (harmonic ? 3 : 0) - (recent.has(seed.id) ? 0.8 : 0)
      - Math.abs(contour(seed.id).energy - tide.wants) * 0.2
      // when the arc asks for an extreme (a break, a hollowing), the gesture that IS that extreme outranks a repertoire
      + (Math.abs(tide.wants) >= 2 && Math.abs(contour(seed.id).energy - tide.wants) <= 0.5 ? 1.8 : 0)
  ...
  // Primary slots are already distinct, but each option then answers itself on a SECOND slot, and until now nothing
  // stopped two options answering on the same one. A round of d3+d6 and d6+d5 cannot be stacked at all: the host
  // refuses with "both of those rewrite d6", which is the performer being offered a choice that was never a choice.
  // Claiming every slot an option touches, primary and companion alike, keeps the options genuinely independent.
  const claimed = new Set(chosen.map(seed => seed.slot));
  return chosen.map((seed, index) => {
    ...
    const answer = companion(seed, input, transformed, excluded, claimed);
    if (answer) claimed.add(answer.part.slot);
    const parts = answer ? [primary, answer.part] : [primary];
    const diff = parts.map(part => `${part.slot} ${part.diff}`).join("  |  ");
    return { origin: "recipe", recipe_id: `ambient-${seed.id}`, slot: seed.slot, code: seed.code, ...(matched.length ? { answers: matched.join(", ") } : {}),
      parts, diff,
      why: `${input.entrance === seed.id ? `${input.dj.name}'s calling card: ` : onTide ? `${tide.says}: ` : ""}${answer ? `${transformed ? `${seed.label} reshapes the room` : `Bring in ${seed.label}`}; ${answer.phrase}.` : seed.why}`,
