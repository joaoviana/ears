...
WARM, NOT OMINOUS. Contrast is in time and register, never in menace. Warmth is a shape: the 200-600 Hz body is the fullest band, the sub sits well under it, the top is present but smooth. A low is a warm body in the D2-A2 register (midinote 38-45), not a 30-50 Hz sub and not a pitch that drops into its note: measured, those read as mud and sound like a horror cue. Never a bare tritone (D against G#), never a lone high note with a long tail, never a muffled rumble behind a near-closed filter, never Gendy tones. Field recordings stay near their own pitch: birds slowed below 0.85 stop being birds. Harmony is welcome when it is \\glow, slow D-lydian major voicings rooted on A2 or D3 with attacks of six to ten seconds that come and go rather than sit.`;
}
...
const TASKS: Record<string, string[]> = {
  ...
  texture: [
    ...
    "Change the depth rather than the surface: move one existing layer behind a soft filter (low-pass no lower than 900 Hz) for a long stretch, then let it surface again.",
