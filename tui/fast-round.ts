...
/** the gestures that carry rhythm: when a round's instant offers have none, the model seat is asked for pulse */
export const RHYTHMIC = new Set(["fingertips", "marbles", "twig-cycle", "pebble-pairs", "leaf-shuffle", "reed-answer",
  "air-flutter", "understone", "brush-burst", "paper-fall", "rain-polyrhythm", "wave-shutters", "swarm", "mist", "pulse"]);

/**
 * Instant proposals are delivered synchronously; the last seat waits on a model. One instant seat by default, so a
 * round reads as "1 is ready now, 2 is Claude Code writing live"; EARS_INSTANT=2 restores the older pair.
 */
export function fastRound(inputs: AskInput[], onOption: (o: Suggestion, agent: string) => void,
  ...
  const offered: Suggestion[] = [], instant = inputs.some(i => i.showcase) ? 2 : Math.max(1, Math.min(2, Number(process.env.EARS_INSTANT ?? 1) || 1)), total = instant + 1;
  ...
  onEvent("composition", { completed: offered.length, total });
  ...
  // a typed note is the brief: the model answers the words, not a rotating musical challenge
  const modelAngle = chosen.note?.trim() ? "answer" : ambientAngles.find(angle => !offeredAngles.has(angle)) ?? "transform";
  ...
  }).finally(() => onEvent("composition", { completed: total, total }));
}
