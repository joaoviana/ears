...
export class LiveGrading extends EventEmitter<{ outcome: [result: GradedShot] }> {
  ...
  private observe = (m: ObservationBody) => {
    ...
    if (previous?.key === key) for (const slot of this.slotIds) {
      const d = slotDifference(previous.slots, slots, slot, { onsets_per_beat: 0, peak_to_envelope_db: 0 });
      if (d) (this.slotNoise[slot] ??= new NoiseFloor()).sample({ ...d.relative_bands_db, brightness: d.centroid_hz, loudness: d.envelope_db });
    }
  ...
  private compare = (m: ComparisonBody) => {
    ...
    const delta = clean && m.differences && shot.slot && PER_SLOT_METRICS.has(shot.expect.metric) && before && after
      ? slotDifference(before.slots, after.slots, shot.slot, m.differences) : null;
    ...
    this.wire.send("outcome", "host", {
      ...
      unit: outcome.unit, noise_floor: outcome.floor, floor_calibrated: noise.ready(shot.expect.metric),
      scope: { kind: delta ? "slot" : "master", per_voice: !!delta, estimate: delta ? "dry-slot contribution: other slots held at their before-measurement; not a controlled re-render" : undefined },
