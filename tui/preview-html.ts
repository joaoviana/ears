...
import fs from "fs"; import { render, LOOKS } from "./ascii.ts";
...
for (const look of LOOKS) {
  ...
  let rows: string[] = [];
  for (let i = 0; i < 30; i++) rows = render(look, "ascii", pal, w, h, 2 + i * 0.066, { ...s, kick: i % 7 === 0 ? 1 : s.kick * Math.exp(-(i % 7) * 0.3), snare: i === 22 ? 1 : 0.3 * Math.exp(-(i - 22) * 0.4) * (i > 22 ? 1 : 0) }, { code });
