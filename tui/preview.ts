...
import { render, LOOKS } from "./ascii.ts";
...
for (const look of LOOKS) {
  ...
  for (let i = 0; i < 10; i++) rows = render(look, look === "waterfall" ? "blocks" : "ascii", "ember", w, h, 3 + i * 0.07, { ...s, kick: i === 3 ? 1 : s.kick * 0.8, snare: i === 2 ? 1 : 0.2 }, { code: "~d.(\\d1, \\instrument, \\kick, \\dur, 1, \\amp, 0.9) ~d.(\\d3, \\instrument, \\bass, \\cutoff, 3800)" });
