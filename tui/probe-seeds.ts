...
e.on("ready", async () => {
  for (let seed = 1; seed <= N; seed++) { const b = makeBase(seed); for (const [k, code] of Object.entries(b.slots)) { if (!code) continue; e.eval(code, `${seed}/${k}`); sent++; } await new Promise((r) => setTimeout(r, 700)); }
