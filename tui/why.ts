...
interface Round { asked: number; angles: Map<string, number>; got: { angle: string; ms: number; slots: string; why: string }[]; lost: { angle: string; ms: number; why: string }[]; verdict?: { t: number; decision: string; by: string }; banked?: boolean }

const rounds: Round[] = [];
let cur: Round | null = null;
for (const m of msgs) {
  if (m.type === "request") {
    if (!cur || cur.verdict) { cur = { asked: m.t, angles: new Map(), got: [], lost: [] }; rounds.push(cur); }
    cur.angles.set(s(m.angle), m.t);
  } else if (m.type === "proposal" && cur) {
    cur.got.push({ angle: s(m.angle) || "banked", ms: Number(m.ms) || m.t - cur.asked, slots: (m.slots || [m.slot]).join("+"), why: s(m.why) });
...
for (const [i, r] of rounds.entries()) {
  ...
  console.log(`${at(r.asked)}  round ${String(i + 1).padStart(2)}  ${r.got.length} option${r.got.length === 1 ? " " : "s"}` +
